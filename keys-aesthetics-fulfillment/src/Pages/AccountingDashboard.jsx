...
export default function AccountingDashboard() {
  ...
  const totalRevenue = orders.filter(o => o.status === 'completed').reduce((sum, order) => sum + order.total_amount, 0);
  const pendingPayments = orders.filter(o => o.status === 'pending').reduce((sum, order) => sum + order.total_amount, 0);
  const monthRevenue = orders.filter(o => {
    const orderDate = new Date(o.createdAt?.seconds * 1000);
    const now = new Date();
    return orderDate.getMonth() === now.getMonth() && orderDate.getFullYear() === now.getFullYear() && o.status === 'completed';
