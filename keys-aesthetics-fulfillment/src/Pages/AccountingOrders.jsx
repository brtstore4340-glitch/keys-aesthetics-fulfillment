...
export default function AccountingOrders() {
  ...
  const totalRevenue = filteredOrders.filter(o => o.status === 'completed').reduce((sum, order) => sum + order.total_amount, 0);
  const pendingAmount = filteredOrders.filter(o => o.status === 'pending').reduce((sum, order) => sum + order.total_amount, 0);
